  ...
  <div class="app-container">
    <el-form :inline="true" :model="searchForm" class="search-form">
      <el-form-item label="订单号">
        <el-input v-model="searchForm.order_sn" placeholder="请输入订单号" style="width: 250px" />
      </el-form-item>
    ...
    <el-table :data="tableData" border v-loading="loading">
      ...
      <el-table-column prop="device_sn" label="机具号" width="150" />
      ...
      <el-table-column label="操作" width="150">
        <template slot-scope="scope">
          <el-button v-if="scope.row.status === 2" size="mini" type="success" @click="handleShip(scope.row)">发货</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { getOrderList, updateOrderStatus } from '@/api/pos'
...
export default {
  ...
  methods: {
    getStatusLabel(status) {
      const map = { 0: '待付款', 1: '已完成', 2: '待发货', 3: '待签收', 4: '待激活' }
    ...
    handleReset() {
      this.searchForm = { order_sn: '', user_id: '', status: '' }
      this.pagination.page = 1
      this.fetchData()
    },
